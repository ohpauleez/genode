...
class Input::Session_component : public Genode::Rpc_object<Session>
{
	...
	public:
		...
		void sigh(Genode::Signal_context_capability sigh) { _from_input->sigh(sigh); }
};
...
class Framebuffer::Session_component : public Genode::Rpc_object<Session>
{
	...
	public:
		...
		:
			_wrapped_framebuffer(wrapped_framebuffer), _fb_ds(fb_ds), _refresh_cnt(0)
		{ }
		...
		{
			_wrapped_framebuffer->sync_sigh(sigh);
		}
};
...
int main(int argc, char **argv)
{
	 ...
	 */
	Nitpicker::Session::View_handle const view_handle = nitpicker.create_view();

	typedef Nitpicker::Session::Command Command;

	Nitpicker::Rect rect(Nitpicker::Point(view_x, view_y),
	 ...
	 */
	static Framebuffer::Session_component fb_monitor(nitpicker.framebuffer(),
	                                                 fb_ds);
	...
	while (true) {
		...
		if (sig_rec.pending()) {
			...
			try {
				config()->reload();
				cfg = Config_args();
				Nitpicker::Rect rect(Nitpicker::Point(cfg.xpos, cfg.ypos),
