...
int main(int, char **)
{
	...
	Framebuffer::Mode const fb_mode(view_width, view_height, nitpicker_mode.format());
	nitpicker.buffer(fb_mode, false);

	static Framebuffer::Session_client framebuffer(nitpicker.framebuffer_session());
	static Nitpicker::View_client      view(nitpicker.create_view());
	...
	view.viewport(MINI_X, MINI_Y, MINI_W, MINI_H, -4, 0, true);
			...
			if (ev.type() == Input::Event::PRESS && key_cnt == 1) {
				...
				case Input::BTN_LEFT:
					view.stack(Nitpicker::View_capability(), true, false);
					{
						...
						view.viewport(x + pad_x, y, view_width - 2*pad_x, view_height, -pad_x, 0, true);
					}
					break;
				default:
					{
